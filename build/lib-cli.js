...
const getLibraryDirectory = (subLibraryDirectory) => {
  return path.resolve(__dirname, '../lib', subLibraryDirectory)
}
...
fs.readdir(getComponentsDirectory(), (err, files) => {
  if (err) {
    ...
    if (files.length > 0) {
      let _files = files.filter(name => name !== 'core' && name !== 'styles' && name !== 'index.js')
      let index = 1, len = _files.length
      _files.forEach(componentName => {
        console.log(chalk.cyan(`Building ${componentName} ${index}/${len}\n`))
        let result = execSync(`vue-cli-service build --target lib --name index --dest lib/${componentName} packages/${componentName}/index.js`)
