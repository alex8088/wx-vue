...
const arrayFrom = (list) => Array.prototype.slice.call(list)

class Carousel {
  constructor (options) {
    this._default = {
      container: '.weui-carousel__box',
      item: '.weui-carousel__item',
      direction: 'vertical',
      activeClass: 'active',
      threshold: 50,
      duration: 300,
    ...
    this.$container = this._options.container.querySelector('.weui-carousel__box')
  ...
  on (event, callback) {
    if (this._eventHandlers[event]) {
      console.error(`[carousel] event ${event} is already register`)
    }
    if (typeof callback !== 'function') {
      console.error('[carousel] parameter callback must be a function')
    }
